...
namespace Gxmc_Gerber
{
	...
	public:
		...
		//显示指定编号的Fov
		void DisplayPadCluster(CDC& dc,ScreenStruct screen,const PadCluster& PadCluster);
		void UpdateMargin(RectD rect,RectD& realMarginRect);
		RectD GetClustersMargin();
	...
	private:
		...
		//从字符串中提取坐标值
		void GetValueFromStr(std::string strX,std::string strY,double& dX,double& dY);
		//读取gerber文件提示信息
		void ReadTipInfo(std::ifstream& infile,char* strBuf,_TipInfo &tipInfo,std::map<std::string,AdStyle>& ADStyleMap);
		//读取直线模式下的插值操作D01
		void ReadLinearIterpolateOperationD01(std::string srcExpression,PointD& point,PointD& CurPoint,GxmcPolygon& polygon,RectD& realMarginRect,bool& isD01Read,bool isFilled,bool isIncludeLine);
		//读取圆弧模式下的插值操作D01
		void ReadCircularIterpolateOperationD01(std::string srcExpression,PointD point,PointD& CurPoint,GxmcPolygon& polygon,RectD& realMarginRect,bool& isD01Read,bool isFilled);
		//读取移动操作D02
		void ReadMoveOpreationD02(std::string srcExpression,PointD point,PointD& CurPoint,GxmcPolygon& polygon,RectD& realMarginRect);
		//读取G75
		void ReadMultiQuadrantModeG75(std::string srcExpression,PointD point,PointD& CurPoint,GxmcPolygon& polygon,RectD& realMarginRect);
        //读取AM参数

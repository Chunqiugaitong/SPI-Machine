...
namespace Gxmc_Gerber
{
		...
		while(true)
		{
			...
			if(nG36>=0)
			{
				...
				{		
					...
					{
						ReadMoveOpreationD02(srcExpression,point,CurPoint,polygon,realMarginRect);
					}				
					//读取D01
					while(true)
					{				
						...
						ReadLinearIterpolateOperationD01(srcExpression,point,CurPoint,polygon,realMarginRect,isD01Read,isFilled,isIncludeLine);
						if (!isD01Read)
						{
							break;
						}	
					...
					{	
						...
						{				
							...
							bool isD01Read = false;
							isFilled = true;
							ReadCircularIterpolateOperationD01(srcExpression,point,CurPoint,polygon,realMarginRect,isD01Read,isFilled);		
							if (!isD01Read)
							{
								break;
							}
                 ...
                 {   
 					...
 					case 2:							
 						if (nFlag == 2)
 						{
 							...
 							GetValueFromStr(strX,strY,point.x,point.y);
 ...
 // 								polygon.m_vecPoint.push_back(point);
	...
	void CGxmcGerber::ReadLinearIterpolateOperationD01(std::string srcExpression,PointD& point,PointD& CurPoint,GxmcPolygon& polygon,
	...
	void CGxmcGerber::ReadCircularIterpolateOperationD01(std::string srcExpression,PointD point,PointD& CurPoint,GxmcPolygon& polygon,
		                                             RectD& realMarginRect,bool& isD01Read,bool isFilled)
	{
		...
		if(nX*nY*nI*nJ*nD01 >= 0 && nG <= 0)
		{
			...
			GetValueFromStr(strX,strY,stopPoint.x,stopPoint.y);
			//圆弧终点更新边界
			UpdateMargin(stopPoint,realMarginRect);
			//读取圆弧中心对圆弧始点偏移量IJ
			GetValueFromStr(strI,strJ,centerPoint.x,centerPoint.y);
			//////////////////////////////////////////////////////////////////////////
			///起始点，没有考虑两次以上点集存在的情况	
			startPoint = CurPoint;
	...
	void CGxmcGerber::ReadMoveOpreationD02(std::string srcExpression,PointD point,PointD& CurPoint,GxmcPolygon& polygon,RectD& realMarginRect)
	{
		int nX = srcExpression.find("X");
		int nY = srcExpression.find("Y");
	...
	void CGxmcGerber::ReadMultiQuadrantModeG75(std::string srcExpression,PointD point,PointD& CurPoint,GxmcPolygon& polygon,RectD& realMarginRect)
	{

	}
	void CGxmcGerber::GetValueFromStr(std::string strX,std::string strY,double& dX,double& dY)
	{
	...
	void CGxmcGerber::ReleaseADStyleMap()
	{
		...
		for ( ; iter != m_adStyleMap.end(); )
		{
			...
			if (iter->second.adInfo.adAmInfo->vecAmCircle.size() > 0 )
			{
				...
				for ( ; itorAmCircle != iter->second.adInfo.adAmInfo->vecAmCircle.end(); )
				{
			...
			if (iter->second.adInfo.adAmInfo->vecAmCenterLine.size() > 0 )
			{
				...
				for ( ; itorAmCenterLine != iter->second.adInfo.adAmInfo->vecAmCenterLine.end(); )
				{
			...
			if (iter->second.adInfo.adAmInfo->vecAmOutLine.size() > 0 )
			{
				...
				for ( ; itorAmOutLine != iter->second.adInfo.adAmInfo->vecAmOutLine.end(); )
				{
			...
			if (iter->second.adInfo.adAmInfo->vecAmPolygon.size() > 0 )
			{
				...
				for ( ; itorAmPolygon != iter->second.adInfo.adAmInfo->vecAmPolygon.end(); )
				{
			...
			if (iter->second.adInfo.adAmInfo->vecAmMoire.size() > 0 )
			{
				...
				for ( ; itorAmMoire != iter->second.adInfo.adAmInfo->vecAmMoire.end(); )
				{
			...
			if (iter->second.adInfo.adAmInfo->vecAmThermal.size() > 0 )
			{
				...
				for ( ; itorAmThermal != iter->second.adInfo.adAmInfo->vecAmThermal.end(); )
				{
...
// 			shape.SizeXmm = itorMapAdstyle->second.adName;
	...
	int CGxmcGerber::ReadFromDatabase()
	{
		...
		//读取Mark点
		vector<CGxmcDatabase::Mark> vecMark;
		m_dataBase.ReadRecordFromTable(CGxmcDatabase::tableList.Mark,CGxmcDatabase::s_markElement,vecMark);
